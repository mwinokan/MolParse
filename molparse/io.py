def write(filename, payload, verbosity=1, **parameters):
    ...
    try:
        ...
        # PBD and amp.System:
        elif (filename.endswith(".pdb") or filename.endswith(".pdb2")) and isinstance(
            payload, System
        ):
...
def read(
    ...
    if verbosity > 0:
        mout.out(
            "reading " + mcol.file + filename + mcol.clear + " ... ", end=""
        )  # user output
    ...
    try:
        ...
        mout.out("Fail.")
        mout.errorOut(
            "Could not read missing file " + mcol.file + filename,
            code="amp.io.read[1]",
            end="",
        )
        return None
...
def parseMol(mol, verbosity=1):
    ...
    if verbosity:
        ...
        mout.out(
            "parsing " + mcol.file + mol + mcol.clear + " ... ", end=""
        )  # user output
...
def parsePDB(
    ...
    try:
        ...
        if index == ":":
            ...
            last_model_line = subprocess.check_output(
                "grep MODEL " + pdb + " | tail -n1 ", shell=True
            )
            ...
            if verbosity > 0:
                mout.out(
                    "parsing " + mcol.file + pdb + mcol.clear + " ... ", end=""
                )  # user output
            ...
            try:
                    ...
                    for line in input_pdb:
                        ...
                        if searching:
                            if line.startswith("MODEL"):
                                ...
                                index == 1 and line[0:6] in ["ATOM  ", "HETATM"]
                            ) or not searching:
                                searching = False
                                ...
                                #### PARSELINE
                                try:
                                    atom = parsePDBAtomLine(
                                        line,
                                        res_counter,
                                        atom_counter,
                                        chain_counter,
                                        debug=debug,
                                        alternative_site_warnings=alternative_site_warnings,
                                    )
                            ...
                            if line.startswith("ENDMDL") and not searching:
                                ...
                                if residue is None:
                                    make_new_res = True
                                if last_residue_name != atom.residue:
                                    make_new_res = True
                                if last_residue_number != atom.res_number:
                                    make_new_res = True
                                if last_chain_name != atom.chain:
                                    make_new_res = True
                                ...
                                if make_new_res:
                                    if residue is not None:
                                        if not resfilter or residue.name in resfilter:
                                            ...
                                            mout.warning(
                                                f"Skipping residue {residue} {residue.number}"
                                            )
                                    residue = new_residue(
                                        atom.residue,
                                        res_counter,
                                    ...
                                    if (
                                        ...
                                        and last_residue_type != residue.type
                                    ):
                                        make_new_chain = True
                                    if make_new_chain:
                ...
                mout.errorOut(
                    "File " + mcol.file + pdb + mcol.error + " not found", fatal=True
                )
            ...
            if num_appended_hydrogen_chains:
                ...
                if reordering_summary_warnings:
                    mout.warning(
                        f"Will try to re-order hydrogens in last {num_appended_hydrogen_chains} chains"
                    )

                assert num_appended_hydrogen_chains < len(system.chains), mout.error(
                    "num_appended_hydrogen_chains >= len(system.chains)"
                )
                ...
                for h_chain in hydrogen_chains:
                    for atom in h_chain.atoms:
                        ...
                        if reordering_warnings:
                            mout.warning(
                                f"Moving {atom} {atom.number} (i) to {residue} {residue.number} (res_indices {atom.res_index}-->{residue.index})"
                            )
                        ...
                        if residue is not None:
                            ...
                            mout.warning(
                                f"Didn't find a home for {atom} {atom.number} ({pdb=})"
                            )
                ...
                for chain in system.chains:
                    for i, res in enumerate(chain.residues):
                        ...
                        if res.number in [r.number for r in chain.residues[:i]]:
                            mout.warning(
                                f"{res} {res.number} has already appeared in chain {chain} {chain.index}"
                            )
...
def parsePDBAtomLine(
    ...
    try:
        ...
        if debug:
            mout.var(f"{atom_index}.name", atom_name)

        residue = line[17:21].strip()
        if debug:
            mout.var(f"{atom_index}.residue", residue)
        ...
        if debug:
            mout.var(f"{atom_index}.pdb_index", pdb_index)
        chain = line[21:22]
        if chain == " ":
            chain = string.ascii_uppercase[chain_counter % 26]
        if debug:
            mout.var(f"{atom_index}.chain", chain)
        res_number = line[22:26].strip()
        if debug:
            mout.var(f"{atom_index}.res_number", res_number)
        ...
        if alternative_site_warnings and alt_site_str:
            mout.warningOut(
                f"Alternative site in PDB! res={residue}, atom={atom_name}, res_number={res_number}"
            )
        if debug:
        ...
        if debug:
            mout.var(f"{atom_index}.position", position)
        ...
        try:
            ...
            occupancy = None
        if debug:
            mout.var(f"{atom_index}.occupancy", occupancy)
        ...
        try:
            ...
            temp_factor = None
        if debug:
            mout.var(f"{atom_index}.temp_factor", temp_factor)

        element = line[76:78].strip()
        if len(element) > 1:
            element = f"{element[0]}{element[1:].lower()}"
        if debug:
            mout.var(f"{atom_index}.element", f"###{element}###")

        chg_str = line[78:80].rstrip("\n")
        if len(chg_str.strip()) < 1:
            chg_str = None
        if debug:
            mout.var(f"{atom_index}.chg_str", chg_str)
        ...
        if line.startswith("HETATM"):
            ...
            hetatm = False
        if debug:
            mout.var(f"{atom_index}.hetatm", hetatm)
...
def parsePDBSSBOND(system, line):
    bond = [
        {
            "chain": line[15].strip(),
            "resname": line[11:14].strip(),
            "resid": int(line[17:21].strip()),
        },
        {
            "chain": line[29].strip(),
            "resname": line[25:28].strip(),
            "resid": int(line[31:35].strip()),
        },
        {
...
def parseGRO(
    ...
    if verbosity > 0:
        mout.out(
            "parsing " + mcol.file + gro + mcol.clear + " ... ", end=""
        )  # user output
        ...
        with open(gro, "r") as input_gro:
            ...
            for line in input_gro:
                ...
                # parse the rest
                    ...
                    # parse an atom line:
                    atom = parseGROAtomLine(
                        line,
                        res_counter,
                        atom_counter,
                        chain_counter,
                        element_guess_warning=element_guess_warnings,
                    )
                    ...
                    if line_counter == 3:
                        ...
                        index = res_counter
                        residue = new_residue(
                            atom.residue, index, atom.res_number, atom.chain
                        )
                    ...
                    if residue is None:
                        ...
                        make_new_chain = True
                    if (
                        auto_ter is not None
                        and make_new_res
                        and last_residue_name in auto_ter
                    ):
                        make_new_chain = True
...
def parseGROAtomLine(
    line, res_index, atom_index, chain_counter, element_guess_warning=True
):
    import string
    from .atom import Atom
    ...
    atom = Atom(
        ...
        velocity=velocity,
        res_index=res_index,
        element_guess_warning=element_guess_warning,
    )
...
def writeCJSON(
    filename, system, use_atom_types=False, gulp_names=False, noPrime=False, verbosity=1
):
    if verbosity > 0:
        mout.out(
            "writing " + mcol.file + filename + mcol.clear + " ... ", end=""
    ...
    data["atoms"] = {
        "names": names,
        "elements": {"number": system.atomic_numbers},
        "coords": {
            "unit": "angstrom",
...
def writePDB(
    ...
    if verbosity > 0:
        mout.out(
            f"writing {mcol.file}{filename}{mcol.clear} ... ", end=""
        )  # user output
    ...
    if not append:
        ...
        if ssbonds:
            if not system.ssbonds:
                mout.warningOut(
                    f"No SS bonds information available. Please run 'system.ssbond_guesser() first'."
                )
            else:
...
def modifyPDB(filename, atoms, copy_from=None):
    ...
    for line in fileinput.FileInput(filename, inplace=1):
        ...
        if (
            pdb_index >= min_index
            and pdb_index <= max_index
            and pdb_index in pdb_indices
        ):
...
def constructPDBAtomLine(atom, index, charges=True, shift_name=False, alt_sites=True):
    ...
    if atom_serial is None:
        mout.warningOut(
            f"{mcol.varName}atom_serial{mcol.clear}{mcol.warning} is None",
            code=f"mp.io.constructPDBAtomLine({atom},{index})",
        )
        atom_serial = index
    if residue_serial is None:
        mout.errorOut(
            f"{mcol.varName}residue_serial{mcol.clear}{mcol.warning} is None",
            code=f"mp.io.constructPDBAtomLine({atom},{index})",
        )
        residue_serial = "    "
    ...
    if atom.residue is None:
        mout.errorOut(
            f"{mcol.varName}atom.residue{mcol.clear}{mcol.warning} is None",
            code=f"mp.io.constructPDBAtomLine({atom},{index})",
        )
    ...
    if atom.chain is None:
        mout.errorOut(
            f"{mcol.varName}atom.chain{mcol.clear}{mcol.warning} is None",
            code=f"mp.io.constructPDBAtomLine({atom},{index})",
        )
        strlist.append(" ")
    else:
...
def parseXYZ(xyz, index=":", verbosity=1):
    ...
    if index == ":":
        ...
        if (num_lines / (num_atoms + 2)) != (num_lines // (num_atoms + 2)):
            mout.errorOut(
                "Wrong number of lines, check XYZ formatting is correct.", fatal=True
            )
            ...
            for j, line in enumerate(input_xyz):
                ...
                if j == 0:
                    ...
                    try:
                        ...
                        mout.warningOut(
                            "XYZ image header string has an unsupported format"
                        )
