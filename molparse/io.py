                        ...
                        if searching:
                            ...
                            if line.startswith("ENDMDL") and not searching:
                                ...
                                break
                                    ...
                                    "# All scores below are weighted scores, not raw scores.") and not searching:
                                break
...
def parsePDBAtomLine(line, res_index, atom_index, chain_counter, debug=False, alternative_site_warnings=True):
    ...
    try:
        ...
        atom = Atom(atom_name, pdb_index, pdb_index, position, residue, chain, res_number, occupancy=occupancy,
                    temp_factor=temp_factor, heterogen=hetatm, charge_str=chg_str, alternative_site=alt_site_str,
                    res_index=res_index, charge=charge, element=element)
...
def constructPDBAtomLine(atom, index, charges=True, shift_name=False, alt_sites=True):
    ...
    if atom_serial is None:
        mout.warningOut(f"{mcol.varName}atom_serial{mcol.clear}{mcol.warning} is None",
                        code=f'mp.io.constructPDBAtomLine({atom},{index})')
        atom_serial = index
    if residue_serial is None:
        mout.errorOut(f"{mcol.varName}residue_serial{mcol.clear}{mcol.warning} is None",
                      code=f'mp.io.constructPDBAtomLine({atom},{index})')
        residue_serial = "    "
    ...
    if atom.residue is None:
        mout.errorOut(f"{mcol.varName}atom.residue{mcol.clear}{mcol.warning} is None",
                      code=f'mp.io.constructPDBAtomLine({atom},{index})')
    ...
    if atom.chain is None:
        mout.errorOut(f"{mcol.varName}atom.chain{mcol.clear}{mcol.warning} is None",
                      code=f'mp.io.constructPDBAtomLine({atom},{index})')
        strlist.append(" ")
    else:
